...
def train_symbol(symbol: str, days: int = 500,
    ...
    emit(f"Fetching {days}d OHLCV data for {symbol}…")
    df = fetch_training_data(symbol, days)
    if df is None or len(df) < 80:
        return {"symbol": symbol, "error": "Not enough historical data (need ≥80 candles)."}

    emit(f"Got {len(df)} candles. Engineering {len(df)-60} feature rows…")
    ...
    emit(f"Done! Accuracy: {metrics['accuracy']*100:.1f}%")
    return metrics


def train_multiple(symbols: list[str], progress_callback=None) -> list[dict]:
    results = []
    for sym in symbols:
        r = train_symbol(sym, progress_callback=progress_callback)
        results.append(r)
    return results
...
def predict_symbol(symbol: str, sentiment_score: float = 0.0, fear_greed: int = 50) -> dict:
    """
    Load trained model, fetch recent data, compute live prediction.
    Returns: {symbol, signal, signal_label, confidence, price, sentiment, fear_greed, error}
    """
    model = TradingModel.load_for(symbol)
    if model is None:
        return {"symbol": symbol, "error": f"No trained model found. Run /train {symbol} first."}

    df = fetch_training_data(symbol, days=120)
